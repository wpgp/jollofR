...
#### Figure 1: Method pipeline overview - input datasets comprise both demographic data and geospatial covariates. Following data preparation/cleaning, statistical models are trained so that predictions can be made at both sampled and unsampled locations. The model outputs include disaggregated population numbers and proportions across various population groups (e.g., age, sex, education, ethnicity, wealth, etc). See Section 5 for more details on the underlying multi-step statistical modelling framework utilised. 
