...
Note that the present version of the package generates subnational age/sex pyramids (or other demographic groups such as ethnicity, wealth indices, etc) and does not generate high resolution age/sex gridded layers. However, the gridded rasters could easily be created from the subnational disaggregated proportions. The version of the package which automatically creates the corresponding high-resolution raster files from the modelled proportions is currently under development by the authors.
...
- **age_proportions.csv**: This file contains the mean disaggregated population prportions by age groups. Variables are written as "prp_age_1, ....,prp_age_n" within the .csv file, where n is the last age group category.
